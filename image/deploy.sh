...
set -eu
export COPYFILE_DISABLE=1   # macOS tar: no ._* files or provenance xattr headers on the Pi side
...
tar -C "$HERE/webapp/dist" --uid 0 --gid 0 --no-xattrs -cf - . | ssh "$HOST" 'sudo find /opt/openflexito/webapp -mindepth 1 -delete && sudo tar -C /opt/openflexito/webapp -xf -'

echo "== device package -> $HOST:$PKG"
tar -C "$HERE/device/openflexito" --uid 0 --gid 0 --no-xattrs --exclude __pycache__ -cf - . | ssh "$HOST" "sudo tar -C $PKG -xf - && sudo find $PKG -name '*.pyc' -delete && /opt/openflexito/venv/bin/python -c 'import openflexito.app' && echo 'import ok'"

echo "== network overlay (wired profile, hotspot fallback script)"
tar -C "$HERE/image/overlay" --uid 0 --gid 0 --no-xattrs -cf - \
