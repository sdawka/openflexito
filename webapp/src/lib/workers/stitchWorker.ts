...
self.onmessage = async (ev: MessageEvent<StitchRequest>) => {
  const { tiles, maxDim, analysisWidth, refine } = ev.data
  try {
    ...
    if (refine && tiles.length > 1) {
      ...
      ;(self as any).postMessage({ progress: `refined ${pairs} overlaps` })
    }
    ...
    for (let i = 0; i < tiles.length; i++) {
      ...
      b.close()
      ;(self as any).postMessage({ progress: `rendered tile ${i + 1}/${tiles.length}` })
    }
    ...
    ;(self as any).postMessage({ result: res })
  } catch (e) {
