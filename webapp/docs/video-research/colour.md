...
- **ASC-CDL grade node** (slope / offset / power / saturation per channel) as a composition step in `store/look.svelte.ts` before the curves, exportable as .cdl; slope 0 on a channel doubles as channel isolation. Belongs in the Look panel, not a video mode.
- **Filmic presets** in the Look (Neutral / Soft ACES via the Narkowicz fit / Flat log-ish with a posterisation warning), applied in linear with an exposure scale ~1.15 and a "preserve hue" option; only meaningful when the camera is set ~0.3–0.5 EV under.
- **Bounded stretch** for auto-levels: never remap more than 25 % of the range, so an empty field is not stretched into noise (the shipped `StableLevels` only has a < 16-code identity guard).
- **Speed presets** for the adaptive parameters (τ 0.5 / 1.5 / 4 s) and a freeze toggle, plus one shared reduced-resolution frame-stats step (histogram, channel means, tile variances) computed once per frame for every processor.
- **Okabe-Ito colour-blind-safe hues** and green/magenta two-channel merges as Look presets; **focus peaking and zebra** as view-target overlays with a self-normalising p95 threshold, frozen while the stage moves.
